...
def store_files_for_years(years, ftype, verbose=False):
    for year in years:
        # renew connection:
...
if __name__ == "__main__":
    ...
    # Determine year:
    years = sys.argv[1]
    start, until = years.split('-')
    if start.isdigit() and until.isdigit():
        print "Searching from " + start + " until " + until
        print "Files of type: " + folder_types[ftype]
        years = map(str, range(int(start), int(until)))
        store_files_for_years(years, ftype, verbose=False)
    else:
        raise ValueError("Please enter a range, e.g. 2009-2014")
